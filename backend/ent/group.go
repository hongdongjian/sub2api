...
type Group struct {
	...
	// OpenAI 分组级请求参数覆盖配置：service_tier/reasoning_effort/text_verbosity
	OpenaiRequestOverrides domain.OpenAIRequestOverrides `json:"openai_request_overrides,omitempty"`
...
func (*Group) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case group.FieldModelRouting, group.FieldSupportedModelScopes, group.FieldMessagesDispatchModelConfig, group.FieldModelsListConfig, group.FieldOpenaiRequestOverrides:
			values[i] = new([]byte)
...
func (_m *Group) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case group.FieldOpenaiRequestOverrides:
			if value, ok := values[i].(*[]byte); !ok {
				return fmt.Errorf("unexpected type %T for field openai_request_overrides", values[i])
			} else if value != nil && len(*value) > 0 {
				if err := json.Unmarshal(*value, &_m.OpenaiRequestOverrides); err != nil {
					return fmt.Errorf("unmarshal field openai_request_overrides: %w", err)
				}
			}
...
func (_m *Group) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("openai_request_overrides=")
	builder.WriteString(fmt.Sprintf("%v", _m.OpenaiRequestOverrides))
	builder.WriteString(", ")
